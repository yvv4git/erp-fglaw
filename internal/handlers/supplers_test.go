...
func TestSupplersHandler_Get(t *testing.T) {
	testCases := []struct {
		name             string
		...
		{
			name: "Go to supplers page",
			request: func() (req *http.Request, err error) {
				req, err = http.NewRequest("GET", "/supplers", nil)
				return
			},
		...
		},
		{
			name: "Page not found",
			request: func() (req *http.Request, err error) {
				req, err = http.NewRequest("GET", "/supplers/unknown-page", nil)
				return
			},
